...
import { ChevronDown, ChevronUp, LogOut, Moon, Sun, User, Shield, X, Save, CheckCircle, Upload, Check, Clock, Palette } from 'lucide-react';
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-border">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 text-sm rounded-lg transition-all ${
              saved ? 'bg-green-500/10 text-green-600 border border-green-500/30' : 'bg-primary text-primary-foreground hover:bg-primary/90'
            }`}
          >
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-card border border-border rounded-xl w-full max-w-lg shadow-xl">
        ...
        <div className="p-6 space-y-3 max-h-[60vh] overflow-y-auto">
          {groups.map(group => (
            <div key={group.id} className={`flex items-center gap-4 p-4 rounded-xl border transition-colors ${
              group.hasAccess ? 'border-primary/20 bg-primary/5' : 'border-border bg-muted/20'
            }`}>
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                group.hasAccess ? 'bg-primary/10' : 'bg-muted'
              }`}>
              ...
              {!group.hasAccess && !requested.has(group.id) && group.canRequest && (
                <button
                  ...
                  className="flex-shrink-0 px-3 py-1.5 text-xs border border-border rounded-lg hover:bg-accent transition-colors whitespace-nowrap"
                >
                  Solicitar acesso
                </button>
              )}
        ...
        <div className="px-6 py-4 border-t border-border bg-muted/10">
          <p className="text-xs text-muted-foreground">
            Solicitações de acesso são enviadas ao administrador e processadas em até 24 horas úteis.
          </p>
        </div>
      </div>
    </div>
...
export function SidebarUserMenu({ isCollapsed, onLogout }: SidebarUserMenuProps) {
      ...
      <div ref={menuRef} className="relative">
        {isCollapsed ? (
          <button
            ...
            title="Dr. Carlos Eduardo Silva · Administrador"
          ...
          <button
            onClick={() => setIsMenuOpen(!isMenuOpen)}
            className="w-full flex items-center gap-3 px-2 py-1.5 hover:bg-accent rounded-lg transition-colors"
          >
            <img
              src={PROFILE_URL}
              alt="Foto do usuário"
            ...
            <div className="flex-1 text-left min-w-0">
              <p className="text-sm font-medium truncate whitespace-nowrap">Dr. Carlos Eduardo Silva</p>
              <p className="text-xs text-muted-foreground truncate whitespace-nowrap">Administrador</p>
            </div>
            {isMenuOpen
        ...
        {/* Dropdown */}
        {isMenuOpen && (
          <div className={`absolute bottom-full mb-2 bg-card border border-border rounded-xl shadow-xl overflow-hidden z-50 ${
            isCollapsed ? 'left-0 w-56' : 'left-0 right-0'
          }`}>
            {/* Cabeçalho do dropdown */}
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-muted/20">
              <img src={PROFILE_URL} alt="Foto" className="w-8 h-8 rounded-full object-cover flex-shrink-0" />
            ...
            >
              ...
              <span
                ...
                style={{ background: THEMES[colorTheme].swatch }}
              />
            </button>
            ...
            <div className="border-t border-border">
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-destructive/10 transition-colors text-left text-sm text-destructive"
              >
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-sm shadow-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              ...
              <button onClick={() => setModal(null)} className="p-1.5 hover:bg-muted rounded-lg transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {(Object.entries(THEMES) as [ColorTheme, typeof THEMES[ColorTheme]][]).map(([key, theme]) => (
                <button
                  ...
                  className={`w-full flex items-center gap-4 p-3.5 rounded-xl border transition-all ${
                    colorTheme === key ? 'border-primary bg-primary/8' : 'border-border hover:bg-accent'
                  }`}
                >
