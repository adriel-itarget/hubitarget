...
import { DollarSign, TrendingUp, TrendingDown, AlertCircle, ChevronDown, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export function FinanceiroDashboard() {
  ...
  const selectedLabel = costCenters.find(c => c.value === selectedCenter)?.label || 'Todos os Centros';
      ...
      <div className="px-8 py-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-start justify-between mb-8">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-card border border-border rounded-lg hover:bg-accent transition-colors text-sm font-medium"
              >
              ...
              {showCenterDropdown && (
                <div className="absolute right-0 top-full mt-1 w-52 bg-card border border-border rounded-xl shadow-lg z-10 overflow-hidden">
                  {costCenters.map(c => (
                    <button
                      key={c.value}
                      onClick={() => { setSelectedCenter(c.value); setShowCenterDropdown(false); }}
                      className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${
                        selectedCenter === c.value ? 'bg-primary/10 text-primary' : 'hover:bg-accent'
                      }`}
                    >
                      {c.label}
                    </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-orange-600 font-medium">98 casos</span>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <div className="bg-card border border-border rounded-xl p-6">
              <h3 className="font-semibold mb-4">Receitas vs Despesas (2026)</h3>
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              <h3 className="font-semibold mb-4">Receita por Centro de Custo</h3>
              <ResponsiveContainer width="100%" height={220}>
                ...
                <div key={t.id} className="px-6 py-4 flex items-center gap-4 hover:bg-muted/30 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      t.status === 'confirmado' ? 'bg-green-500/10 text-green-600' : 'bg-yellow-500/10 text-yellow-600'
                    }`}>
                      {t.status}
                    </span>
                  </div>
